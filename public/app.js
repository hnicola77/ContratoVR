...
async function carregarContratos() {
  try {
    ...
    document.getElementById('tabelaContratos').innerHTML = 
      '<tr><td colspan="8" style="text-align:center; color: #ef4444;">Erro ao carregar contratos</td></tr>';
  }
}
...
function renderizarTabela() {
  ...
  contratosFiltrados.forEach(contrato => {
    ...
    if (contrato.tipo_servico === 'piso') tipoBadge = 'Piso';
    else if (contrato.tipo_servico === 'azulejo') tipoBadge = 'Azulejo';
    else if (contrato.tipo_servico === 'ambos') tipoBadge = 'Piso + Azulejo';
      ...
      <td>
        <div style="width: 100%; background: #e2e8f0; border-radius: 4px; height: 20px; overflow: hidden;">
          <div style="width: ${percentualExecutado}%; background: linear-gradient(90deg, #10b981, #059669); height: 100%; display: flex; align-items: center; justify-content: center; color: white; font-size: 11px; font-weight: bold;">
            ${percentualExecutado > 10 ? percentualExecutado + '%' : ''}
          </div>
        </div>
      </td>
      <td>
...
function verDetalhes(id) {
  alert('Tela de detalhes em desenvolvimento. Contrato ID: ' + id);
}

function editarContrato(id) {
  alert('Tela de edição em desenvolvimento. Contrato ID: ' + id);
}

function distribuirMetragem(id) {
  alert('Tela de distribuição em desenvolvimento. Contrato ID: ' + id);
}
